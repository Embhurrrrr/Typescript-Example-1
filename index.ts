...
let cityListView: CityInterface[] = JSON.parse(
...
export const searchCities = (searchCity: string) => {
	const filteredCities = cityListView.filter((city) => {
		const lowerSearchCity = searchCity.toLowerCase();
		return (
			city.cityName.toLowerCase().includes(lowerSearchCity) ||
...
const moveZerosToEnd = (arr: any[]): any[] => {
